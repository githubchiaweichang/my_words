# -pend懸掛

# 来自拉丁语 pendere, 悬挂，词源同 pendulum.
pending--0,即將發生的,待定的
pendulum,鐘擺,擺錘,搖擺不定的人或事態
# 反复称量的，反复衡量的，引申词义哀伤的
pensive--0,憂鬱沉思的

# per- 贯穿 , 通过 + -pend- 垂
perpendicular--0,垂直

# s ( ex- )出 , 向外 + -pend- 支付
spend,花費,浪費,度過,消耗,消磨,獻出,花費,用盡
spending,開銷,花費
spendthrift,揮金如土的人,揮霍的,類,狂熱者熱情浪費者浪費,英雄不是怯懦的揮霍者不是吝嗇的,吝嗇鬼是吝嗇的揮霍者是揮霍的

# 前缀 ap- 同 ad-. 词根 pend, 悬
appended,附加的

# com-, 强调。 -pend, 悬垂
compendium--0,簡要,彙編
compensate,動詞,補償,賠償,酬報
compensation,補償賠償物,補償賠償金
compensatory,賠償的,補償的,補充的
recompense,酬謝,回報,賠償,回來,共同,衡量,動作

# de- 下降 + -pend- 悬挂
dependable,可信任的,可靠的,可信賴的
depended,相信,信賴
dependent,形容詞,依靠的,依賴的,名詞,受撫養者
independence,獨立,自主
independent,獨立的,自治的

# dis-, 分开，散开。 -pens, 悬挂，称量
dispensable--0,可有可無的,不必要的,記法,不,花費,不值得花費的
dispense--0,分配,分發,施與,試行配,藥,發,藥
indispensability,必不可少的
indispensable--0,不可缺少之物

# ex-, 向外。 -pend, 花销
expend,花費,消耗,支出,海把,暫時不用的繩,繞在杆上
expenditure,花費,使用,尤指金錢的,支出額
expense,花費,消費,消耗,開支,經費,費錢之物

# im-, 进入，使，上面， -pend, 悬挂
impending--0,即將發生的,迫在眉睫的
inexpensive,形容詞,花費不多的,價錢低廉的
interdependence--0,互相依賴

#sus- 下 , 低 + -pend- 悬挂
suspend,懸,吊,使懸浮,暫停,中止,推遲,暫停,中止,懸浮,宣佈破產
suspended,暫停的,緩期的,宣判,懸浮的
suspense,懸疑,不安,懸而未決的狀態
suspension,名詞,暫停,中止,停職